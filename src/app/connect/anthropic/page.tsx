...
function AiKeyStep() {
  ...
  useEffect(() => {
    fetch('/api/user/ai-config')
      ...
      .then(d => {
        if (!d) return
        const providers = d.providers as Record<string, { hasKey: boolean; hasEnvFallback: boolean }>
        setHasAnyKey(Object.values(providers).some(p => p.hasKey))
        setHasServerDefault(Object.values(providers).some(p => p.hasEnvFallback))
      })
      .catch(() => {})
  }, [])
